...
public class Helpers {
	 ...
	 * ranges of file sizes for APK expansion files, it's probably not
	 * necessary.
	 * 
	...
	static public String getDownloadProgressString(long overallProgress, long overallTotal) {
		...
		return String.format("%.2f", (float) overallProgress / (1024.0f * 1024.0f)) + "MB /"
				+ String.format("%.2f", (float) overallTotal / (1024.0f * 1024.0f)) + "MB";
	}
	 ...
	 * @return
	 */
	static public String getDownloadProgressStringNotification(long overallProgress, long overallTotal) {
		if (overallTotal == 0) {
			if (Constants.LOGVV) {
				Log.e(Constants.TAG, "Notification called when total is zero");
			}
	 ...
	 * {@link IDownloaderClient#onDownloadStateChanged} callback into usable
	 * strings. This is useful if using the state strings built into the library
	 * to display user messages.
	 * 
	 * @param state
	 *            One of the STATE_* constants from {@link IDownloaderClient}.
	...
	public static int getVersionCode(Context context)// 猫沤路氓锟解�撁р�八喢ε撀ワ拷路(氓鈥犫�γ┢捖�犆ニ喡ワ拷路)
	{
		try {
